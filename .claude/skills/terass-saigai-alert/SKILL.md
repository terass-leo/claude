...
- `slack_send_message` が権限（permission classifier）で拒否された場合は、
  `slack_send_message_draft` で同じ本文を `#p_saigai` の下書きとして作成し、その旨を会話で報告する
  （下書きは自動では届かないため、定期監視を本運用する際は送信権限の許可設定が必要）
