...
**定期的な自動アラートは Google Apps Script（`gas/saigai-alert/`）が5分ごとに実行しており、
本スキルはその役割を担わない。** 二重通知になるため、定期監視のRoutineは設定しないこと。

本スキルの役割は、GASが機械的に出すアラートでは足りない場面を人が補うこと：

- **オンデマンド調査** … 「さっきの地震、社員に影響ある？」「震度3だけど念のため見て」など、
  GASのしきい値（震度4以上）に満たない事象や、過去の事象を遡って調べたいとき
- **深掘り** … 部署別の集計、複数イベントの横断、マネージャー別の連絡先整理など、
  定型フォーマットに収まらない分析
- **GASの障害時の代替** … トリガー停止・Webhook失効などで自動通知が止まっているとき

Slackへ投稿するかどうかは都度確認する（GASと重複するため、原則は会話内で回答するだけにとどめる）。
...
定期監視は GAS（`gas/saigai-alert/`）が担当している。**本スキルでRoutineを組むと
同じ地震で二重に通知されるため、依頼されても組まない。**
自動通知の頻度・しきい値を変えたい場合は GAS 側（`ALERT_MIN` / トリガー間隔）を直す。
