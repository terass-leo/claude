...
var MASTER_HEADERS = [
  '氏名', '区分', '締結日', '参画予定日', '応募チャネル', '出身企業', '経歴・実績',
...
function buildSnapshot() {
  ...
  var focus   = buildFocusMonth_(live, monthly);
...
function readMaster_() {
  ...
  for (var r = 1; r < values.length; r++) {
    ...
    out.push({
      ...
      career: String(row[col['経歴・実績']] || '').trim(),
...
/**
 * 「今月の決定」ブロック用。当月の HC / WB と目標差分、決定者の内訳を返す。
 * 当月がまだ0名なら、直近で決定のあった月に切り替えて内訳を見せる（空欄にしない）。
 */
function buildFocusMonth_(live, monthly) {
  ...
  if (m.hc === 0) {
    for (var i = cur - 1; i >= 0; i--) {
      if (monthly[i].hc > 0) { target = i; fallback = true; break; }
    }
  }

  var members = live.filter(function (d) { return d.signMonth === target; })
    .sort(function (a, b) { return String(a.signDate).localeCompare(String(b.signDate)); })
    .map(function (d) {
      return {
        name: d.name, tier: d.tier, wb: d.wb, slot: d.slot,
        origin: d.origin, career: d.career,
        joinLabel: d.joinLabel || '未定', signDate: d.signDate,
        expect: d.expect, guarantee: d.guarantee, fixed: d.fixed, signup: d.signup,
        commit: d.guarantee + d.fixed + d.signup
      };
    });
  ...
  return {
    monthLabel: (cur + 1) + '月',
    hc: m.hc, wb: m.wb,
    hcTarget: m.hcTarget, wbTarget: m.wbTarget,
    hcDiff: m.hc - m.hcTarget,
    wbDiff: round1_(m.wb - m.wbTarget),
    avgWb: m.avgWb,
    members: members,
    membersMonthLabel: (target + 1) + '月',
    isFallback: fallback,
    careerMissing: members.filter(function (x) { return !x.career; }).length
  };
}
...
// ============================================================
var SEED_DECIDED = [
  ['小田 俊介', 'ピカイチ', '2026/03/05', '2026/05/01', 'リストマッチ', 'REDS（不動産流通システム）',
   '住友不動産販売→三井住友トラスト→REDS。REDSの全国No.1セールス・売上レコード保持者。2025年度実績5,000〜6,000万円（会社方針で手数料半額のため正規換算1億円相当）。',
   '大河原さん', 3300, 1500, 0, 0, '締結', ''],
  ['藤岡 将也', 'ピカイチ', '2026/03/07', '2026/05/01', 'リストマッチ', '住友不動産販売',
   '住友不動産販売在籍。全国50位・年間売上1.5億円規模のトップセールス。ピカイチ鈴木さんからのリファラルで参画。',
   '鈴木英徳さん', 2400, 1400, 120, 0, '締結', '固定給 30万×4か月／集客費用 15万×4か月'],
  ['吉川 航二', 'ピカイチ', '2026/04/01', '2026/09/01', 'リストマッチ', '住友不動産販売',
   '住友不動産販売在籍（藤岡さんとほぼ同じ店舗出身）。全国50位・年間売上1.5億円規模。30代前半。大河原さんからの紹介。6月ボーナス受領後の参画を条件に締結。',
   '大河原さん', 3000, 1500, 0, 0, '締結', ''],
  ['藤崎 常博', '準ピカイチ', '2026/05/06', '2026/10/01', '直接応募', '住友不動産販売',
   '住友不動産販売在籍。売上期待値2,600万円（準ピカイチ水準）。子供誕生を機に自宅購入を兼ねて秋参画予定。地場仲介から案件をもらって決めた人物。',
   '', 2400, 1700, 0, 0, '締結', ''],
  ['田邊 柚樹', 'ピカイチ', '2026/05/23', '2026/08/01', 'リストマッチ', '住友不動産販売',
   '住友不動産販売在籍。20年近くのキャリアでほぼ収益・事業系案件に特化。昨年度売上1億円・年収3,000万円。仲介スキル・実績ともに高評価。',
   '谷津さん', 2400, 2000, 0, 0, '締結', ''],
  ['塚越 翔太', 'ピカイチ', '2026/05/27', '2026/09/01', 'リファラル', '住友不動産販売',
   '住友不動産販売在籍（武蔵小杉エリア）。年間売上1億4,000万円。土地戸建て・マンションすべて対応可能。集客戦略を構築できるマインドを評価。重道さんからのリファラル。',
   '重道さん', 2400, 1500, 0, 0, '締結', ''],
  ['常光 孝博', '準ピカイチ', '2026/05/27', '2026/09/01', 'リストマッチ', '東宝ハウス',
   '東宝ハウス在籍。売上期待値2,500万円（準ピカイチ水準）。新規参画の東宝出身TAにピカイチを紹介してもらい、候補者バイネームを挙げてもらった声掛け作戦から内定承諾。',
   '福田さん', 2400, 1200, 0, 0, '締結', ''],
  ['鳥越 篤', 'その他', '2026/05/30', '2026/09/01', 'リストマッチ', '住友不動産販売（湘南エリア）',
   '住友不動産販売湘南エリア在籍。売上期待値2,000万円程度。岡部さんからのリファラルで昨年7月にインタビューフックで紹介され、1年近くのフォローを経て承諾。集客費用補填の提案が決断の後押しとなった。',
   '岡部さん', 2100, 0, 150, 0, '締結', '固定給 25万×6か月'],
  ['鈴木 碧', '準ピカイチ', '2026/06/23', '', 'リストマッチ', '三菱UFJ不動産販売',
   '締結後に辞退。配偶者の反対により現職残留。次回接点は2026/10/14を予定。',
   '谷津さん', 0, 0, 0, 0, '辞退（締結後）', 'HC・WBの集計対象外'],
  ['小畑 慧伍', 'ピカイチ', '2026/06/30', '2026/09/01', 'リストマッチ', '住友不動産販売',
   '住友不動産STEP出身。住友内では1億以上の売上実績。TERASSでの売上期待値は3,000万円以上。紹介をベースに集客が成り立つ優秀な営業マン。谷津さん紹介からの承諾。',
   '谷津さん', 2400, 1800, 0, 0, '締結', ''],
  ['中川 雅史', 'ピカイチ', '2026/07/01', '2026/08/01', 'リストマッチ', '住友不動産販売',
   '住友不動産STEP出身。鈴木さんからの紹介。住友では所長として勤務。埼玉内ではずっとトップセールス。業者との繋がりも強く売上期待値は高い。',
   '鈴木さん', 3000, 2000, 0, 200, '締結', 'サインアップ 200万'],
  ['西野 大智', '準ピカイチ', '2026/07/02', '2026/09/01', 'リストマッチ', '住友不動産販売',
   '住友不動産STEP出身。谷津さん会食からの決定。英語が話せ海外顧客の対応が得意。売上期待値は2,500万円程度。本人の年収保証への期待値が低く1,100万円で着地。奥様も住友の営業で売上はさらに高く、こちらも狙いにいく。',
   '谷津さん', 2400, 1100, 0, 0, '締結', ''],
  ['岡田 雅美', 'ピカイチ', '2026/07/07', '2026/08/01', 'リストマッチ', '住友不動産販売',
   '住友不動産STEP出身。神奈川の超ビッグネーム。自社サイト経由で山元がアプローチしていたが、AAの藤井さんがフォローして再面談となった。ベッタリの業者からの集客が強く、1億以上の売上期待値あり。',
   '', 4200, 0, 0, 0, '締結', '保証なし'],
  ['池田 悠真', '準ピカイチ', '2026/07/15', '2026/08/01', 'リストマッチ', '住友不動産販売',
   '住友不動産STEP出身。谷津さん会食からの決定。比較的若手で売上期待値は2,500万円程度。年収保証ではなく固定給にうまくスライドさせられた。',
   '谷津さん', 2400, 0, 120, 0, '締結', '固定給 30万×4か月'],
  ['吉村 剣郎', 'その他', '2026/07/14', '2026/09/01', 'リストマッチ', '住友不動産販売',
   '住友不動産STEP出身。谷津さん会食からの決定。東京西側エリアで実績を出して都心に呼ばれた優秀層。比較的若手で売上期待値は1,500万円程度。行動力の担保が必要。',
   '谷津さん', 2100, 1000, 0, 0, '締結', ''],
  ['安彦 詠太', 'ピカイチ', '2026/07/27', '2026/09/01', 'リストマッチ', '住友不動産販売',
   '住友不動産STEP出身。鈴木さんからの紹介。鈴木さんが「自分が抜けた後のビッグネーム」と評するほど優秀。住友での売上は2億程度。7月中旬の面談で承諾を得て9月1日参画で着地させられた好事例。',
   '鈴木英徳さん', 0, 1900, 0, 0, '締結', '売上期待値が未登録']
];
